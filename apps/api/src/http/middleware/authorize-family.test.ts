...
describe("authorizeFamily", () => {
  it("uses the route familyId and ignores a forged x-family-id header", async () => {
    const authorize = vi.fn(async (_userId: string, familyId: string) => ({
    ...
    expect(response.body.data.familyId).toBe(routeFamilyId);
    expect(authorize).toHaveBeenCalledWith(userId, routeFamilyId, undefined);
    expect(authorize).not.toHaveBeenCalledWith(userId, forgedHeaderFamilyId, expect.anything());
  });
