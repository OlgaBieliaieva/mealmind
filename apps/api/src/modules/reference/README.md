...
б одне поле. `DELETE` виконує безпечний soft delete й повертає оновлене
значення. Фізичне видалення довідників не підтримується:
...
Ті самі status-поля можна передати через `PATCH` для відновлення. Деактивація
product category проходить hierarchy validation і відхиляється, якщо категорія
ще має активних нащадків.
