...
Документ поєднує реалізовану технічну основу та цільові межі наступних vertical slices. Фактичний стан конкретної можливості визначається кодом, тестами й явно позначеним статусом у відповідному розділі.
...
- onboarding і роботу з єдиною активною сім’єю MVP;
...
- підтвердження фактичного споживання й перегляд особистої статистики;
- сімейний dashboard виконання плану в межах authorization policy.
...
Реалізована persistence foundation містить 56 прикладних моделей, одну reviewed baseline migration, 181 детермінований reference record і окремі clean-database tests. Повний опис наведено в розділі [«Модель даних MealMind»](./data-model/README.md).
...
Модуль визначає стабільний порядок, правила унікальності та структуру
ієрархічних довідників. Адміністративний UI використовує маршрути
`/reference/:resource`; create, update, archive та restore завжди повторно
авторизуються API. `DELETE` реалізований як resource-specific soft delete без
фізичного видалення пов’язаних записів.
...
- optimistic revision і конфліктами паралельних змін;
...
Один `MealEntry` може мати кількох `MealEntryParticipant`; batch кількох днів
і профілів зберігається атомарно. Позиція може мати незалежну позначку
готовності (`preparedAt`), але статуси фактичного споживання або пропуску не
належать плану: вони з’являються лише у щоденнику чи resolution.

### Consumption Diary

Модуль володіє фактами споживання, їх історичною харчовою цінністю та зіставленням із планом.

Він відповідає за:

- обчислювані планові кандидати без передчасного створення фактів;
- підтвердження, коригування та пропуск запланованих порцій;
- ручне додавання продуктів і рецептів із каталогу;
- відокремлення життєвого циклу плану від факту;
- nutrient snapshot, версію розрахунку та completeness metadata;
- особисту денну статистику;
- періодний family-scoped dashboard із дотриманням плану, фактичними
  нутрієнтами, персональними цілями та динамікою ваги.

`ConsumptionEntry` є фактом і не замінює `MealEntryParticipant`. Зв’язок із планом є optional і забезпечує простежуваність; незапланована позиція такого зв’язку не має. Щоденник дня не зберігається як окремий aggregate root, а будується як read model за `localDate`.
...
- масштабування кількості інгредієнтів відповідно до сумарної запланованої
  ваги страви; у shopping calculation v1 знаменником є сума
  RecipeIngredient.gramWeight, а Recipe.yieldWeightG не використовується;
- включення продуктів, доданих до плану безпосередньо;
- нормалізацію сумісних одиниць вимірювання;
- агрегацію однакових продуктів;
- групування позицій за безпосередньою батьківською категорією продукту, якщо
  вона існує;
...
Список є persisted versioned snapshot. Display metadata продукту й категорій,
generation warnings, derived quantity та source fingerprint фіксуються під час
явної generation command. Звичайне читання не перераховує snapshot, а лише
визначає stale state порівнянням fingerprint.
...
Планові порції відображаються як обчислювані кандидати. Підтвердження створює окремий факт споживання з посиланням на джерело, якщо воно існує.

Для кожного факту система зберігає:

- фактичну кількість і нормалізовану масу;
- локальну дату, час і timezone snapshot;
- optional зв’язок із плановою порцією;
- snapshot підтримуваних нутрієнтів;
- версію розрахунку й ознаку повноти;
- audit metadata, status і revision.

Порівняння «план / факт» не змінює жодну зі сторін. `PENDING`, `CONFIRMED`, `CHANGED` і `SKIPPED` є станами read model або окремої resolution, а не новими статусами планової сутності. Непідтверджений запис не означає, що учасник не їв.

Сімейна статистика агрегує статуси окремо за учасниками. Фактичні калорії чи нутрієнти різних людей не підсумовуються в єдину норму сім’ї.

У плані харчування допускається окремий операційний підсумок нутрієнтів усіх
вибраних порцій, але він не є «сімейною нормою» і не використовується для
оцінки балансу. Відповідність цілям розраховується лише в персональній
projection профілю з активним `NutrientTargetSet` на кожну дату періоду.
Персональна projection не стискає діапазони: разом з опорним значенням вона
повертає мінімальну, точну й максимальну межі, а UI показує заплановане
значення поруч із відповідною ціллю для кожного доступного макронутрієнта.

У персональному представленні частка енергії окремого прийому їжі може
показуватися лише як довідковий відсоток від добової енергетичної цілі
відповідного профілю та дати. Це не є розподілом цілі між прийомами їжі, тому
meal-level progress bar і висновки про відповідність не формуються. Без
активної добової цілі відображається тільки енергія в абсолютному значенні.

### Приготування

Для recipe-based позиції плану backend створює snapshot інгредієнтів і кроків, після чого приймає idempotent команди зміни їх стану. Стан `PENDING` блокує завершення, але не означає пропуск.

Фіналізація виконується транзакційно:

1. перевіряє family access і актуальну revision;
2. переконується, що всі інгредієнти й кроки явно опрацьовані;
3. нормалізує фактичні кількості використаних і замінених інгредієнтів;
4. розраховує nutrient snapshot із completeness і calculator version;
5. фіксує фактичний вихід та переводить сеанс у `COMPLETED`.

Завершений сеанс не редагується звичайними mutation-командами. Корекція вимагає окремої задокументованої політики, щоб не переписувати факти споживання.
...
4. підсумувати вагу participant portions для кожної страви й масштабувати всі,
   включно з optional, інгредієнти відносно суми їх gramWeight;
5. додати безпосередньо заплановані продукти;
6. нормалізувати лише сумісні одиниці;
7. агрегувати однакові продукти;
8. згрупувати результат за parent category першого рівня або власною
   категорією, якщо parent відсутня;
...
- OWNER може читати й змінювати щоденники всіх членів власної сім’ї;
- MEMBER не читає щоденники інших учасників, а application administrator не отримує такого доступу через свою глобальну роль;
...
- перевірка ідемпотентності reference seed;
...
- [доменні ERD](./data-model/erd.md);
- [словник даних](./data-model/data-dictionary.md);
- deployment diagram;
- ADR index;
